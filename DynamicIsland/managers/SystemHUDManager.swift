...
class SystemHUDManager {
    ...
    func setup(coordinator: DynamicIslandViewCoordinator) {
        ...
        // Start observer if any HUD/OSD is enabled and we are not in dictation-only mode
        if !Defaults[.dictationOnlyMode] && (Defaults[.enableSystemHUD] || Defaults[.enableCustomOSD] || Defaults[.enableVerticalHUD] || Defaults[.enableCircularHUD]) {
            Task { @MainActor in
    ...
    @MainActor
    func updateObserverStateForDictationMode() async {
        if Defaults[.dictationOnlyMode] {
            await stopSystemObserver()
        } else if Defaults[.enableSystemHUD] || Defaults[.enableCustomOSD] || Defaults[.enableVerticalHUD] || Defaults[.enableCircularHUD] {
            await startSystemObserver()
        }
    }
