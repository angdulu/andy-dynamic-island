...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("dynamic.island", image: "TrayIcon", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    private func cleanAllDynamicIslandWindows() {
        for window in windows.values {
            window.close()
            NotchSpaceManager.shared.notchSpace.windows.remove(window)
        }
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if Defaults[.enableScreenRecordingDetection] && !Defaults[.dictationOnlyMode] {
            ScreenRecordingManager.shared.startMonitoring()
        }
        
        // Setup Do Not Disturb Manager
        if Defaults[.enableDoNotDisturbDetection] && !Defaults[.dictationOnlyMode] {
        ...
        if Defaults[.enableRealTimeWaveform] && !Defaults[.dictationOnlyMode] {
            Task {
        ...
        Defaults.publisher(.enableRealTimeWaveform, options: [])
            .sink { [weak self] change in
                if change.newValue && !Defaults[.dictationOnlyMode] {
                    Task {
                        await AudioTap.shared.startCapture()
                    }
    ...
    private func installTopMenuItemsIfNeeded() {
        ...
        // Remove Services menu to prevent KeyboardShortcuts conflicts with system services like "Search man Page"
        if let appMenu = mainMenu.items.first?.submenu {
            if let servicesItem = appMenu.items.first(where: { $0.title == "Services" || $0.submenu?.title == "Services" }) {
    ...
    private func registerOptionalShortcutHandlers() {
        ...
        KeyboardShortcuts.onKeyDown(for: .screenAssistantPanel) {
            guard Defaults[.enableShortcuts], Defaults[.enableScreenAssistant] else { return }

            DictationManager.shared.handleShortcutKeyDown()
        }

        KeyboardShortcuts.onKeyUp(for: .screenAssistantPanel) {
            guard Defaults[.enableShortcuts], Defaults[.enableScreenAssistant] else { return }

            DictationManager.shared.handleShortcutKeyUp()
        }
    }
    ...
    private func updateBackgroundMonitoringStates() {
        let dictationOnly = Defaults[.dictationOnlyMode]
