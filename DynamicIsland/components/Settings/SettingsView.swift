...
private enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    var title: String {
        ...
        case .screenAssistant: return String(localized: "Dictation")
    ...
    var systemImage: String {
        ...
        case .screenAssistant: return "mic"
...
struct SettingsView: View {
    ...
    private var settingsSearchIndex: [SettingsSearchEntry] {
        [
            ...
            SettingsSearchEntry(tab: .general, title: "Enable Andy", keywords: ["andy", "pet", "notch"], highlightID: SettingsTab.general.highlightID(for: "Enable Andy")),
    ...
    private func isTabVisible(_ tab: SettingsTab) -> Bool {
        switch tab {
        case .timer, .stats, .clipboard, .shelf, .terminal:
    ...
    private func detailView(for tab: SettingsTab) -> some View {
        ...
        case .about:
            if let controller = updaterController {
                ...
                SettingsForm(tab: .about) {
                    About(updaterController: SPUStandardUpdaterController(startingUpdater: true, updaterDelegate: nil, userDriverDelegate: nil))
                }
            }
        }
    }
...
struct GeneralSettings: View {
    ...
    @Default(.enableMinimalisticUI) var enableMinimalisticUI
    @Default(.enableAndyFeature) var enableAndyFeature
    ...
    var body: some View {
        Form {
            ...
            Section {
                Defaults.Toggle(key: .enableAndyFeature) {
                    Text("Enable Andy")
                }
                .onChange(of: enableAndyFeature) { _, isEnabled in
                    if !isEnabled && coordinator.currentView == .andy {
                        coordinator.currentView = .home
                    }
                }
                .settingsHighlight(id: highlightID("Enable Andy"))
            } header: {
                Text("Andy")
            } footer: {
                Text("Turn off Andy's notch tab and idle notch presence while keeping the rest of Atoll active.")
            }
        ...
        Form {
            ...
            if enableShortcuts {
                ...
                Section {
                    HStack {
                        VStack(alignment: .leading) {
                            KeyboardShortcuts.Recorder("Transcribe Shortcut:", name: .screenAssistantPanel)
                                .disabled(!enableShortcuts || !Defaults[.enableScreenAssistant])
                            if !Defaults[.enableScreenAssistant] {
                                Text("Dictation feature is disabled")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                                    .padding(.top, 2)
                            }
                    ...
                    Text("Starts or stops voice transcription. Only works when dictation is enabled.")
                        .multilineTextAlignment(.trailing)
                        .foregroundStyle(.secondary)
                        .font(.caption)
                }
...
struct TimerSettings: View {
    ...
    private var timerFeatureSection: some View {
        Section {
            ...
            Text("Control whether the app mirrors timers started from the macOS Clock app and live activity behavior.")
        }
    }
    ...
    var body: some View {
        Form {
            Section {
                ...
                Defaults.Toggle(key: .dictationOnlyMode) {
                    Text("Dictation-Only Mode (Disable Dynamic Island)")
                }
                .settingsHighlight(id: highlightID("Dictation-Only Mode"))
                .disabled(!enableScreenAssistant)
            } header: {
                Text("Dictation")
            } footer: {
                Text("Disables all Dynamic Island features (notch UI, media controllers, custom system HUDs, etc.), leaving only dictation active to conserve system resources.")
            }
            ...
            if enableScreenAssistant {
                Section {
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Transcribe Shortcut")
                            Text("Configure the shortcut in Settings > Shortcuts.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Push To Talk")
                            Text("Hold the shortcut to record, then release to transcribe.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                        Toggle("", isOn: $dictationPushToTalk)
                            .toggleStyle(.switch)
                    }
                    ...
                    Text("When Push To Talk is off, the shortcut will toggle recording on and off.")
                }
                ...
                Section {
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("Play native dictation sounds when starting and stopping recording.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                        Toggle("", isOn: Binding(
                            get: { Defaults[.dictationSoundEffects] },
                            set: { Defaults[.dictationSoundEffects] = $0 }
                        ))
                        .toggleStyle(.switch)
                    }
                    .settingsHighlight(id: highlightID("Play Sound Effects"))
                }
                ...
                Section {
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Paste Method")
                            Text(dictationPasteMethod.description)
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                        Spacer()
                        Picker("", selection: $dictationPasteMethod) {
                            ForEach(DictationPasteMethod.allCases, id: \.self) { method in
                    ...
                    .settingsHighlight(id: highlightID("Paste Method"))
                } header: {
                    Text("Output")
                } footer: {
                    Text("Direct types at the active cursor without changing the clipboard. Direct + Clipboard inserts and saves the transcript. Clipboard only copies it.")
                }
            }
        }
        .formStyle(.grouped)
