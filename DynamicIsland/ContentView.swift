...
struct ContentView: View {
    ...
    @State private var isClosedAndyWingCollapsed = false
    ...
    private var shouldShowClosedAndyWing: Bool {
        closedAndyCanOccupyIdleSlot && !isClosedAndyWingCollapsed
    }

    private func applyClosedAndyWingCollapsed(_ collapsed: Bool) {
        hoverTask?.cancel()
        coordinator.suppressHoverOpen(for: 1.0)

        withAnimation(.smooth) {
            isClosedAndyWingCollapsed = collapsed
            isHovering = false
          ...
          VStack(alignment: .leading) {
              VStack(alignment: .leading) {
                  if coordinator.firstLaunch {
                      ...
                      } else if shouldShowClosedAndyWing {
                          ...
                          switch coordinator.currentView {
                            ...
                            case .andy:
                                if enableAndyFeature {
                                    NotchAndyView(
                                        isClosedAndyWingCollapsed: $isClosedAndyWingCollapsed,
                                        setClosedAndyWingCollapsed: applyClosedAndyWingCollapsed
                                    )
                                } else {
                                    NotchHomeView(albumArtNamespace: albumArtNamespace)
                                }
...
private struct NotchAndyView: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            AndyNotchWebView(
                rootURL: AndyNotchResources.rootURL,
                stateManager: stateManager,
                menuActions: AndyNotchWebViewMenuActions(
                    isCollapsed: {
                        isClosedAndyWingCollapsed
                    },
                    reload: {
                        reloadToken = UUID()
                    },
                    showInNotch: {
                        setClosedAndyWingCollapsed(false)
                    },
                    hideFromNotch: {
                        setClosedAndyWingCollapsed(true)
                    }
                )
            )
                .id(reloadToken)
...
struct ClosedAndyWing: View {
    ...
    var body: some View {
        AndyNotchWebView(rootURL: AndyNotchResources.rootURL, stateManager: stateManager)
            ...
            .scaleEffect(max(size / 80 * 0.93, 0.01))
            .frame(width: size, height: size)
            .clipShape(RoundedRectangle(cornerRadius: max(size / 2, 1), style: .continuous))
            .background(
                RoundedRectangle(cornerRadius: max(size / 2, 1), style: .continuous)
                    .fill(Color.black)
            )
            .clipped()
    }
