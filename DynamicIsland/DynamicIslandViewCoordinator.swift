...
class DynamicIslandViewCoordinator: ObservableObject {
    ...
    private init() {
        ...
        Defaults.publisher(.enableAndyFeature)
            .receive(on: DispatchQueue.main)
            .sink { [weak self] change in
                self?.handleAndyFeatureToggle(change.newValue)
            }
            .store(in: &cancellables)
    ...
    private func handleAndyFeatureToggle(_ isEnabled: Bool) {
        guard !isEnabled, currentView == .andy else { return }
        withAnimation(.smooth) {
            currentView = .home
        }
    }
